...
当前统一模型规定：汇总节点不能作为申报入口，规则项必须作为申报入口。学委或学院统一上传也必须选择规则项，并转换成该规则项的一条受信申报。

为避免同一业务出现多种表达，节点职责严格分离：

- `aggregate` 才能设置 `aggregation_type` 和 `max_score`，只读取直接子节点得分。
- `item` 的 `aggregation_type` 和 `max_score` 必须为空，基础分只来自 `rule_calculation_config`。
- 计分配置、申报字段、材料要求、审核要求和团体分配规则只能挂在 `item`。
- 一个汇总节点若只有一个子节点，通常没有真实汇总意义，发布规则版本时会被拒绝。
- 同一成果内部互斥的级别、奖项和角色可以作为一个规则项的分级选项；可能同时发生的不同成果必须拆成多个平级规则项，再由父汇总节点处理。
...
- 规则项固定 `allow_repeat = 0`，不能配置为允许重复。
- 发布时检查规则项是否有计分配置，并拒绝单子节点的冗余汇总层。
...
`application_record` 建立唯一键 `(academic_year_id, student_id, rule_item_code)`。因此一名学生在一个学年内对同一规则项最多只有一条申报。退回后修改原记录，不能新建第二条；撤回或驳回也不会释放唯一名额。接口会先返回友好的 `409`，数据库唯一键负责拦截并发请求。
...
审核详情接口同时返回 `application_attachment`。前端根据 `mime_type` 对 PDF 使用内嵌阅读器、对图片使用等比预览，其他文件提供新窗口打开链接。
